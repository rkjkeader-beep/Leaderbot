...
# ── Throttle signaux ────────────────────────────────────────────
MAX_SIG_PER_HOUR  = 3   # max 3 signaux par heure glissante
...
def claude_validate_signal(sig: dict, session: str, htf_trend: str) -> dict:
    ...
    validated = (verdict == "VALIDER"
                 ...
                 # timing_ok retiré comme bloqueur — devient avertissement uniquement
...
def agent_analyze(m, score_min, news_ok, q):
    ...
    try:
        ...
        # ── Liquidité M15 (BONUS/PÉNALITÉ — plus obligatoire) ────────
        liq = agent_liquidity(m15, b)
        if liq:
            sc = min(sc + liq["score"], 115)   # +15 à +25 pts si liquidité confirmée
        else:
            sc = max(0, sc - 10)               # -10 pts si absente (pénalité légère)
            liq = {"type": "NONE", "level": 0, "score": 0, "label": "⚠️ Liq non confirmée"}
...
def _scan_inner():
    ...
    for sig, key in sigs:
        ...
        if now_check.hour >= SIGNAL_CUTOFF_HOUR:
            log("WARN", clr("Signal {} bloqué — après 22h00 UTC".format(sig["name"]), "y"))
            continue

        # ── Throttle global : max 3/h, max 10/j, gap 15min ──────────
        ok_send, reason_throttle = _throttle_allowed(now_check)
        if not ok_send:
            log("WARN", clr("Signal {} bloqué THROTTLE — {}".format(sig["name"], reason_throttle), "y"))
            continue
